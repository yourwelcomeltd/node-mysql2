  ...
  if (options.processDots === true && !options.nestTables && !options.rowsAsArray) {
    ...
    for (let i = 0; i < fields.length; i++) {
      if (fields[i].name.indexOf('.') !== -1) {
        const fieldNames = fields[i].name.split('.');
        if(fieldNames.length > 1) {
          // Remove the actual field name
          fieldNames.pop();

          // Escape each part of the field path
          fieldNames.forEach((part, index, namesArray) => {
            namesArray[index] = helpers.srcEscape(part);
          });
    ...
    if (allObjects.length > 0) {
      ...
      for (let i = 0; i < allObjects.length; ++i) {
        const fieldNames = allObjects[i];
        parserFn(`result[${fieldNames.join('][')}] = {};`);
      }
    }
  }
